...
(function (global, factory) {
	...
	(global = typeof globalThis !== 'undefined' ? globalThis : global || self, global.EagerForm = factory());
	...
	*/
	 ...
	 * @return {Object}
	 */
	function referenceRule (element, attribute) {
	  return new Promise((resolve, reject) => {
	...
	function remoteRule (element, attribute) {
	  let endpoint = element.getAttribute(attribute).replace("{value}", element.value);
	  ...
	  if (element.hasAttribute("".concat(attribute, "-options"))) {
	    try {
	      customOptions = JSON.parse(element.getAttribute("".concat(attribute, "-options")));
	    } catch (err) {}
	  }
	  ...
	  return new Promise((resolve, reject) => {
	    ...
	    xhr.onerror = function () {
	      console.log("".concat(this.status, " ").concat(this.statusText));
	    };
	...
	var enLocale = {
	  ...
	  tooLong: "Please shorten this text to {maxlength} characters or less (you are currently using {count} characters).",
	  tooShort: "Please lengthen this text to {minlength} characters or more (you are currently using {count} characters).",
	...
	class EagerForm {
	  ...
	  constructor(element, options = {}) {
	    ...
	    this.submitBtn = this.form.querySelector('[type="submit"]'); // The elements to be selected by this.getSiblings

	    this.selectorElement = "input, select, textarea"; // The selector expression used by this.getSiblings

	    this.selector = ':input:not([type="hidden"]):not([type="submit"]):not([type="reset"]):not(button)'; // To store settimeouts
	    ...
	    this.resetHandler = this.handleReset.bind(this); // One instance per form element, sorry
	  ...
	  setError(element, validityType) {
	    ...
	    element.classList.add(this.options.classes.inputInvalidClass); // Remove valid class
	    ...
	    if (parent) {
	      ...
	      parent.classList.add(this.options.classes.parentInvalidClass); // Also remove the valid class from parent

	      parent.classList.remove(this.options.classes.parentValidClass); // Since, parent exists, let's find sibling inputs within the parent
	      ...
	      if (siblings.length > 1) {
	        ...
	        for (let index = 0; index < siblings.length; index++) {
	          ...
	          siblings[index].classList.add(this.options.classes.inputInvalidClass); // .. and remove the valid class

	          siblings[index].classList.remove(this.options.classes.inputValidClass);
	        }
	      }
	  ...
	  clearError(element) {
	    ...
	    if (parent) {
	      ...
	      if (siblings.length > 1) {
	        for (let index = 0; index < siblings.length; index++) {
	          siblings[index].classList.remove(this.options.classes.inputInvalidClass);
